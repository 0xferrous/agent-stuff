/**
 * Taken from: https://github.com/hjanuschka/shitty-extensions/blob/9075b2d7647f6d0a3bc4bccc4382797828a2a1b5/extensions/usage-bar.ts
 *
 ...
 * {
 *   "usageBar": {
 *     "providers": ["anthropic", "copilot", "gemini", "codex", "antigravity", "kiro", "zai"]
 *   }
 * }
  ...
  if (!apiKey) {
    ...
    try {
      ...
      if (fs.existsSync(authPath)) {
        ...
        apiKey = auth["z-ai"]?.access || auth["zai"]?.key || auth["zai"]?.access;
      }
    } catch {}
  }
...
function loadSettings(): UsageBarSettings {
  // Try project settings first, then global settings
  const settingsPaths = [
  ...
  for (const settingsPath of settingsPaths) {
    try {
      if (fs.existsSync(settingsPath)) {
        ...
        if (data.usageBar) {
          return data.usageBar as UsageBarSettings;
        }
      }
    } catch {}
  }
...
class UsageComponent {
  ...
  private settings: UsageBarSettings;
  ...
  private async load() {
    ...
    const allProviders = ["anthropic", "copilot", "gemini", "codex", "antigravity", "kiro", "zai"];
    const providersToFetch = enabledProviders
      ? allProviders.filter((p) => enabledProviders.includes(p))
      : allProviders;

    // Map provider names to fetch functions
    const providerMap = {
      anthropic: fetchClaudeUsage,
      copilot: () => fetchCopilotUsage(this.modelRegistry),
      gemini: () => fetchGeminiUsage(this.modelRegistry),
      codex: () => fetchCodexUsage(this.modelRegistry),
      antigravity: () => fetchAntigravityUsage(this.modelRegistry),
      kiro: fetchKiroUsage,
      zai: fetchZaiUsage,
    };

    // Map provider names to status fetch functions
    const statusMap = {
    ...
    const usagePromises = providersToFetch.map((provider) =>
      timeout(
    ...
    // Fetch statuses for providers that support them
    const statusPromises = providersToFetch
      .filter((p) => p in statusMap)
      .map((provider) =>
        timeout(
          statusMap[provider as keyof typeof statusMap](),
          3000,
    ...
    // Attach status to usage (match by provider)
    let statusIndex = 0;
    for (const usage of usages) {
      const statusProvider = providersToFetch.find((p) => p in statusMap && p === usage.provider);
      if (statusProvider) {
    ...
    // Filter providers:
    // - Always show explicitly enabled providers (with their errors if any)
    // - For auto-discovery (no providers specified), only show configured ones
    if (enabledProviders) {
      // User explicitly specified which providers to show - show all of them
      this.usages = usages;
    } else {
      // Auto-discovery mode: only show providers that have data
      this.usages = usages.filter(
        (u) =>
          u.windows.length > 0 ||
